...
1) **Quickstart (10 minuti)**  
   👉 `docs/01-Quickstart_AI_Studio.md`

2) **Installazione e configurazione MCP / Bridge (Claude Desktop / Codex)**  
   👉 `docs/02-Installazione_Config_MCP_Bridge.md`

3) **Risoluzione problemi (FAQ)**  
   👉 `docs/04-Troubleshooting_FAQ.md`
...
1) Vai al repository scripts (esempi, regole, community):  
   👉 **MasterSap Python Scripts**: https://github.com/AMV-Team/MasterSap_Python_Scripts

2) Consulta il riferimento comandi MasterPy (MS.*):  
   👉 `docs/03-Reference_MasterPy_Commands.md`

> MasterPy è l’ambiente per script Python arricchiti con comandi specifici MS (materiali, sezioni, nodi, travi, piastre, macro, ecc.) e richiede attenzione a ASCII-only e case-sensitive.
